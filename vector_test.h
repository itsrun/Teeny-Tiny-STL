...
void vector_test() {
	...
	API_TEST_CUSTOM(mv, sv, mv.insert(mv.cbegin(), mv2.rbegin(), mv2.rend()), sv.insert(sv.cbegin(), sv2.rbegin(), sv2.rend()));
	API_TEST11(mv, sv, erase, begin, , 2);
	API_TEST01(mv, sv, pop_back, );
	vector<int> mv3(mv2);
	std::vector<int> sv3(sv2);
	API_TEST_EACH01(mv, sv, swap, mv3, sv3);
	vector<int> mv4(std::move(mv3));
	std::vector<int> sv4(std::move(sv3));
	API_TEST_EACH01(mv, sv, swap, mv4, sv4);
	mv.clear();
	sv.clear();
	...
	API_TEST_CUSTOM(msv, ssv, msv.insert(msv.cbegin(),msv2.rbegin(),msv2.rend()), ssv.insert(ssv.cbegin(), ssv2.rbegin(), ssv2.rend()));
